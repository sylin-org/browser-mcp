...
| [0051](0051-verification-topology-fewer-moving-parts.md) | Fewer, more meaningful moving parts in the verification topology: harness hygiene first (`GHOSTLIGHT_AUDIT_DIR` isolation, serial + closed-stdin E2E tier, structured-state assertions over log-text, a centralized advertised-surface oracle), then merge the two thin adapters into one `ghostlight-relay --role agent\|browser` (3 executables -> 2), parameterize the relay's stdio for in-process testing, and re-tier the ~74 spawn tests down to ~12 irreducible E2E; the majority of wiring tests move onto the existing `Browser`-over-`duplex` in-process seam | Proposed (companion to `docs/design/verification-topology-evaluation.md`; refines [0046](0046-role-specific-executables.md)/[0003](0003-tokio-native-ipc.md)) |
