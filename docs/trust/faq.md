...
This is the front door. Each answer is written to be quoted: the first paragraph stands on
its own for pasting into an assessment portal, links follow, and a closing evidence line
names the artifact behind the claim. The recurring theme is that Ghostlight's runtime
executes on your infrastructure, so most vendor-side risk questions have a structural rather
...
telemetry, and initiates no network activity beyond your own tool calls and the audit
destinations you configure. Page content, credentials, audit records, and policy all stay on
...
There is no model-provider client in Ghostlight's dependency tree. Whichever model your MCP client uses,
and whatever that provider's training terms are, is a relationship between you and that
provider that Ghostlight neither mediates nor observes.
...
third party to process on its behalf. The only third parties in the picture are ones you
select and control: the MCP client and model you run, the SIEM you stream audit to, and the
endpoint that hosts your central policy.
...
Yes, a DPA template is published, and it leads with the structural fact that the vendor
processes no customer personal data. Because the runtime never sends personal data to a
Ghostlight service, the conventional controller-processor mechanics do not engage; the DPA
records that plainly rather than papering over a data flow that does not exist. Your
...
the agent may do to which hosts; observe, shadow, and enforce modes control whether actions
run at all; and a panic kill switch stops everything immediately. Injection can still mislead
...
by capability (read, action, write, execute) and gated accordingly; observe, shadow, and
enforce modes let you run the agent in a watching or simulated posture before granting it real
actions. A take-the-wheel pause hands control back to the human mid-run, and a panic kill
...
Yes. Each tool call produces an identity-bound audit record, and under managed governance that
...
rules applied at that moment. Audit streams to syslog (RFC 5424 over UDP) or JSON Lines files
today; HTTP delivery is deferred.
...
policy. You can watch a device activate a local managed policy end to end:
...
these frameworks apply to your deployment is a determination for you and your counsel. The
controls page maps the specifics.
...
user's own hardened Chrome, in place, under your policy, and never moves the session anywhere.
There is no separate browser to sanction, and because every agent action is attributed in the
audit trail, the automation is more accountable than an unlogged human doing the same clicks,
not less.
...
(documented decisions, runnable scenarios, source-available governance code) and a
certification roadmap that tracks the customer base. The honest inventory is on the trust
center landing page.
...
The assets that matter on the vendor side are the source repository, the release signing keys,
and the release pipeline, and those are what we protect. Signing keys are held offline on an
air-gapped machine and never touch CI or any online system; source and pipeline access uses
multi-factor authentication and least privilege; and changes reach a release only through
reviewed decision records, CI gates, and signed artifacts. There is no customer-data store on
our side to defend, so our security effort concentrates on the integrity of what we ship to
you.

See [supply-chain.md](supply-chain.md).

Evidence: supply-chain.md (build and change management); ADR-0028 Decision 10 (air-gapped signing).
...
that enforces policy rather than trust a summary of it. Suspected vulnerabilities go through
...
with the affected versions and the remediation. Incidents inside your own deployment are yours
to detect through the audit trail Ghostlight produces.

See [security-overview.md](security-overview.md).

Evidence: security-overview.md (inverted incident-response framing; advisory window); ADR-0057 Decision 11a.
...
the governance module is source-available, you hold the actual code, which is stronger than a
source-escrow arrangement that only releases on a trigger. You can prove the license-expiry
behavior yourself:
...
Central policy is served from a last-known-good cache that keeps enforcing even when your
policy source is unreachable, and a cold boot with nothing available fails closed to the
protective state rather than opening up. Your continuity therefore depends on your own
infrastructure, which you already plan for, not on ours. You can watch the fail-closed cold
boot directly:
...
Yes. Each release ships a CycloneDX software bill of materials generated in the release
pipeline, alongside per-file SHA-256 checksums and build-provenance attestations, across the
package managers Ghostlight distributes through. You can verify what you downloaded against the
published checksums and confirm its provenance before deploying. The dependency tree is
deliberately lean, and the cryptography is pure-Rust with no native TLS stack in the default
build.
...
Every extension permission is justified individually in the permission document, so your
security team can review the exact access before approving it. The extension is Manifest V3
and ships no remotely hosted code, which is what enterprise review and force-install policies
expect. Distribute it to your fleet through the Chrome Web Store listing and Chromium's
`ExtensionInstallForcelist`, pinned to the reviewed extension ID.
...
they go through the private channel in SECURITY.md.
...
Ghostlight is open-core. The automation engine is licensed Apache-2.0 OR MIT, which is an
open source license; the governance module is source-available under the Ghostlight
Commercial License, so you can read and audit its code even though that license is not. At
