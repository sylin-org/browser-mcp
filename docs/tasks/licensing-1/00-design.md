...
(Amended 2026-07-04: SECURITY.md landed outside the batch and is never-touch.) SBOM: two
steps inserted before `.github/workflows/release.yml`'s upload step (install
`cargo-cyclonedx`, generate), plus the SBOM file added to the upload step's `path:`
list; exact YAML pinned in l05.
