...
//! registry ([`config`]), the stable denial id scheme ([`denial`]), the policy manifest
//! ([`manifest`]), the audit flight recorder ([`audit`]), and the policy-decision-point/
//! policy-enforcement-point contract ([`ports`]). The dependency direction is strictly inward:
