...
//! renderer ([`explain`]), the audit-replay policy simulator ([`simulate`]), the policy
//! manifest ([`manifest`]), the audit flight recorder ([`audit`]), and the
//! policy-decision-point/policy-enforcement-point contract ([`ports`]). The dependency
//! direction is strictly inward: infra and the browser plugin may depend on this module; this
//! module depends only on std and serde (plus `uuid`/`chrono`/`sha2` for audit, manifest
//! identity, and denial ids). A fail-closed arch-test (task A7) enforces that.
