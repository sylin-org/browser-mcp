...
/// The 20 tool names in advertised order (the 13 trained tools plus `wait_for`, `script`,
/// `form_fill`, `file_upload` (ADR-0050 Decision 2), `browser_batch` (ADR-0050 Decision 3),
/// `upload_image` (ADR-0050 Decision 4), and ADR-0022 Decision 7's sanctioned `explain` addition,
/// positioned last), copied from the code-declared registry (`browser::directory::REGISTRY`), in
/// declared order.
const GOLDEN_TOOL_NAMES: [&str; 20] = [
    "tabs_context_mcp",
    "tabs_create_mcp",
    "navigate",
    "computer",
...
fn tools_list_is_byte_stable_through_the_move() {
    ...
    assert_eq!(
        ...
        "all 20 tools advertised (13 trained plus wait_for, script, form_fill, file_upload, browser_batch, upload_image, and explain)"
    );
