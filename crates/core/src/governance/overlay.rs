...
use crate::browser::{pattern, polarity};
...
use crate::governance::ports::{Capability, Decision, EffectiveMode, GoverningResource, NullSink};
...
impl SessionOverlay {
    ...
    /// manifest is (`pattern::is_valid_pattern`) -- into a tighten-only session tier. A parse or
    /// shape error is returned verbatim so the client learns precisely what was malformed; the
    /// caller declines the overlay rather than silently proceeding without it.
    pub fn parse(text: &str) -> Result<Self, ManifestError> {
        let manifest = parse_manifest(text, "session-overlay", pattern::is_valid_pattern)?;
...
mod tests {
    ...
    fn parses_a_schema_3_overlay() {
        let overlay = SessionOverlay::parse(sylin_only_overlay()).expect("valid overlay parses");
        assert_eq!(overlay.sacred_domains(), &["sacred-bank.invalid".to_string()]);
    }

    #[test]
    fn allows_a_granted_host() {
        let overlay = SessionOverlay::parse(sylin_only_overlay()).unwrap();
        assert!(matches!(
    ...
    fn a_malformed_overlay_is_rejected_not_ignored() {
        ...
        let err = SessionOverlay::parse(r#"{"schema": 2, "name": "x", "version": "1", "grants": []}"#);
        assert!(err.is_err());
    }
    ...
    fn an_overlay_without_sacred_domains_reports_none() {
        ...
        let overlay = SessionOverlay::parse(text).unwrap();
